...
Completed training entries live in `src/data/caseStudies.ts`. Eric confirmed publication of FDQ Microsoft 365 Copilot training and the completed 12-week AI Vision Community course on 11 September 2026. Add only completed engagements supported by confirmed facts. Omit unknown metrics, testimonials and client details instead of publishing empty templates.

Eric confirmed on 11 September 2026 that AI Vision Consulting is not VAT registered and authorised publication. `src/data/pricingPolicy.ts` records `vatConfirmed: true`; displayed prices are final with no VAT added. Case studies include only confirmed completed training. Accreditation remains false until an actual award is documented. External agent sync is separate from website publication.
