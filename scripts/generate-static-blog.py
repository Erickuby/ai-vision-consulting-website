...
CATALOGUE_PATH = ROOT / "src" / "data" / "blog.ts"
SITE_URL = "https://aivisionconsulting.co.uk"
GENERATED_MARKER = "<!-- Generated by scripts/generate-static-blog.py -->"
...
def parse_catalogue() -> dict[str, dict]:
    """Read the shared article catalogue so related links stay in one place."""
    ...
    for block in re.findall(r"\{\n(.*?)\n  \},", text, flags=re.S):
        ...
        if "slug" not in values:
            continue
        tags_match = re.search(r"^\s+tags:\s*\[(.*?)\],?$", block, flags=re.M)
        values["tags"] = re.findall(r"['\"](.*?)['\"]", tags_match.group(1)) if tags_match else []
        cards[values["slug"]] = values
    if not cards:
        raise ValueError(f"No articles parsed from {CATALOGUE_PATH}")
    return cards


def related_articles(slug: str, catalogue: dict[str, dict]) -> list[dict]:
    """Rank other articles by category match, then shared tags, then recency."""
    current = catalogue.get(slug, {})
    current_tags = set(current.get("tags", []))

    def score(card: dict) -> tuple:
        shared = len(current_tags & set(card.get("tags", [])))
        same_category = card.get("category") == current.get("category")
        return (2 if same_category else 0) + shared, card.get("publishedAt", "")

    others = [card for card_slug, card in catalogue.items() if card_slug != slug]
    others.sort(key=score, reverse=True)
    return others[:RELATED_COUNT]


def related_section(slug: str, catalogue: dict[str, dict]) -> str:
    cards = related_articles(slug, catalogue)
    if not cards:
        return ""
    items = []
    for card in cards:
        image_path = urlparse(card["image"]).path
        items.append(
            f'<a class="related-card" href="/blog/{html.escape(card["slug"], quote=True)}">'
            f'<img src="{html.escape(image_path, quote=True)}" alt="" width="1600" height="900" loading="lazy" decoding="async">'
            '<span class="related-card__body">'
            '<span class="related-card__meta">'
            f'<span class="related-card__category">{html.escape(card["category"])}</span>'
            f'<span>{html.escape(card["readTime"])}</span>'
            '</span>'
            f'<span class="related-card__title">{html.escape(card["title"])}</span>'
            f'<span class="related-card__excerpt">{html.escape(card["excerpt"])}</span>'
            '</span></a>'
        )
    return (
        f'{RELATED_START}'
        '<section class="related-posts reveal" aria-labelledby="related-articles">'
        '<h2 id="related-articles">Related articles</h2>'
        '<p>More practical guides from AI Vision Consulting.</p>'
        f'<div class="related-grid">{"".join(items)}</div>'
        f'</section>{RELATED_END}'
    )


def apply_related(page: str, slug: str, catalogue: dict[str, dict]) -> str:
    """Insert or refresh the related block just before the article closes."""
    section = related_section(slug, catalogue)
    if not section:
        return page
    if RELATED_START in page and RELATED_END in page:
        return re.sub(
    ...
    if anchor not in page:
        raise ValueError(f"{slug}: could not find the article body close to attach related articles")
    return page.replace(anchor, f"            {section}\n{anchor}", 1)
...
def generate() -> None:
    ...
    for source in sources:
        ...
        page = apply_related(page, meta["slug"], catalogue)
    ...
    for output in OUTPUT_DIR.glob("*.html"):
        ...
        if GENERATED_MARKER in text:
            output.unlink()
            continue
        # Hand-authored pages keep their body, but their related links are
        # refreshed here so a new article never leaves an older page stale.
        refreshed = apply_related(text, output.stem, catalogue)
        if refreshed != text:
            output.write_text(refreshed, encoding="utf-8")
            print(f"refreshed related articles on /blog/{output.stem}")
